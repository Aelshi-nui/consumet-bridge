...
/** Open page, wait for Cloudflare challenge to pass, intercept API response. */
  ...
  try {
    const result = await Promise.race([
      new Promise((resolve, reject) => {
        setTimeout(() => reject(new Error("timeout after 40s")), TIMEOUT);
        page.on("response", async resp => {
          if (resp.url().includes(matchPath) && resp.status() === 200) {
            try { const j = await resp.json(); resolve(j); } catch {}
          }
        });
        page.goto(url, { waitUntil: "domcontentloaded", timeout: TIMEOUT })
          .then(async () => {
            // Wait for CF challenge to clear (title changes from "..." to real title)
            for (let i = 0; i < 20; i++) {
              const title = await page.title().catch(() => "...");
              if (title && title !== "..." && !title.toLowerCase().includes("just a moment")) break;
...
async function paheSources(episodeFullId) {
  ...
  try {
    page.on("response", resp => {
      const u = resp.url();
      if (u.includes(".m3u8") && u.startsWith("https")) m3u8Urls.add(u);
    });
...
async function handleInfo(req, res) {
  ...
  try {
    let paheId = id;
    if (/^\d+$/.test(id)) {
      const r = await paheSearch(id);
      if (!r.length) return fail(res, `Could not resolve ${id}`, 404);
      paheId = r[0].id;
    }
...
async function handleWatch(req, res) {
  ...
  try { const s = await paheSources(episodeId); ok(res, { sources: s, subtitles: [] }); }
  catch (e) { fail(res, e.message); }
}
