 ...
 * Installs Playwright Chromium on startup if missing, then scrapes AnimePahe.
 */
import { execSync } from "node:child_process";
import { existsSync } from "node:fs";

// Set browser path to /tmp which is writable on Railway
process.env.PLAYWRIGHT_BROWSERS_PATH = "/tmp/pw-browsers";

// Install chromium if not already present
const marker = "/tmp/pw-browsers/.installed";
...
async function fetchPageJson(url, matchPath) {
  ...
  try {
    const hit = await new Promise((resolve, reject) => {
      ...
      page.goto(url, { waitUntil: "networkidle", timeout: 22000 }).catch(reject);
    });
    return hit;
...
async function paheSources(episodeFullId) {
  ...
  try {
    ...
    await page.goto(PAHE + "/play/" + paheId + "/" + epSession, { waitUntil: "networkidle", timeout: 25000 });
